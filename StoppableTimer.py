...
class StoppableTimer:
    ...
    def  __init__(self, time_s):
        ...
        self._time_left_ns = time_s * 1000 * 1000 * 1000
    ...
    def run_timer(self):
        ...
        print("timer started seconds left", self._time_left_ns // (1000 * 1000 * 1000))
        while self._time_left_ns >= 0:
            if self.state == "READY":
                time_start_ns = time.time_ns()
            real_elapsed_s = (time.time_ns() - time_start_ns) // (1000 * 1000 * 1000)
            tp_ns = time.time_ns() - last_timestamp_ns
            print(f"state : {self.state} - real : {real_elapsed_s} - timer : {self._time_left_ns // (1000 * 1000 * 1000)}")
            self._time_left_ns -= tp_ns * self.time_factor
            last_timestamp_ns = time.time_ns()
